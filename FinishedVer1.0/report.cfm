...
<cfparam name="Form.code" default="no" >
<cfquery name="projD" datasource="r2d2" >

SELECT emp_id FROM employee_util where charge_code = '#Form.code#'AND
		  month = '#Form.sMonth#' AND
		  year = '#Form.sYear#'

</cfquery>
 <cfset allPerson = valueList(projD.emp_id) />



<cfset allPersonArray = listToArray(allPerson) />



<cfloop array= #allPersonArray# index=idx>
